...
There are two things to consider in this algorithm.  First, the loop through steps 2 and 3 must eventually terminate because each time you go through it, you replace $a_{11}$ by a divisor of $a_{11}$.
This cannot continue indefinitely, so eventually you will reach step 4.  If $a_{11}$ divides everything in the lower submatrix, then by induction, once that matrix is in standard form, the whole
